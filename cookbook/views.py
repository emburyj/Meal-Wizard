...
def meal_plan_detail_view(request, rcid):
    mp = RecipeCollection.objects.get(pk=rcid) # this gives you the mp object
    meal_plan_q = MealPlan.objects.filter(rec_col__exact=mp) # this gives a query obj
    ...
    context = {'recipes_ingredients': recs_ings}
